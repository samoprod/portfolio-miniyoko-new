...
function forms() {
	...
	if ($('.cart-item').length > 0) {
		$('.cart-item').each(function () {
			var pr = parseInt($(this).find('.cart-table-price .rub').text()) * parseInt($(this).find('.quantity__input').val());
			$(this).find('.cart-table-sum .rub').text(pr);
		});
	}
	$('.quantity__btn').click(function (event) {
		...
		$(this).parent().find('.quantity__input').trigger("change");
		return false;
	});
	...
	$('.quantity__input').on("change", function () {
		...
		if ($(this).closest('.cart-item').length > 0) {
			var pr = parseInt($(this).closest('.cart-item').find('.cart-table-price .rub').text()) * n;
			$(this).closest('.cart-item').find('.cart-table-sum .rub').text(pr);
		}
...
function adaptive_header(w, h) {
	...
	var headerCats = $('.categories-list');
	var headerContacts = $('.header-btn, header .contacts-minbox');


	var cartTotal = $('.vouch-tot-wrap');
	if (w < 767) {
		if (headerCats.length > 0) {
			if (!headerCats.hasClass('done')) {
				headerCats.addClass('done').appendTo(headerMenu);
			}
		}
		...
		if (headerCats.length > 0) {
			if (headerCats.hasClass('done')) {
				headerCats.removeClass('done').appendTo('.categories');
			}
		}
		if (headerContacts.hasClass('done')) {
			headerContacts.removeClass('done').appendTo('.header-line-wrap');
		}
	...
	if (w < 993) {
		if (!cartTotal.hasClass('done')) {
			cartTotal.addClass('done').appendTo(".checkout-inpts-wrap");
		}
	}
	else {
		if (cartTotal.hasClass('done')) {
			cartTotal.removeClass('done').appendTo('.vouch');
		}
	}
}
...
if($('.slider-top').length>0){
	var h = 0;
	setTimeout(function(){
		h = parseInt($('.categories').outerHeight())
		if(h < 380){ h = 380; }
		$(".slider-top").height(h);
	}, 300);
	$('.slider-top').slick({
		...
		autoplaySpeed: 3000,
		fade: true,
		nextArrow:'<button type="button" class="slick-next"><svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.28418 1L10.5684 6.28423L5.28418 11.5685" stroke="#303841" stroke-linejoin="round"/></svg></button>',
		prevArrow:'<button type="button" class="slick-prev"><svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.71582 1L1.4316 6.28423L6.71582 11.5685" stroke="#303841" stroke-linejoin="round"/></svg></button>',
		responsive: [{
			breakpoint: 768,
			settings: {}
		}]
...
if($('.reviews-slider').length>0){
	...
	slider.slick({
		...
		centerMode: true,
		centerPadding: "150px",
		nextArrow:'<button type="button" class="slick-next"><svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M16.7071 8.70711C17.0976 8.31658 17.0976 7.68342 16.7071 7.29289L10.3431 0.928932C9.95262 0.538408 9.31946 0.538408 8.92893 0.928932C8.53841 1.31946 8.53841 1.95262 8.92893 2.34315L14.5858 8L8.92893 13.6569C8.53841 14.0474 8.53841 14.6805 8.92893 15.0711C9.31946 15.4616 9.95262 15.4616 10.3431 15.0711L16.7071 8.70711ZM0 9H16V7H0V9Z" fill="#878787"/></svg></button>',
		prevArrow:'<button type="button" class="slick-prev"><svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0.292893 8.70711C-0.0976311 8.31658 -0.0976311 7.68342 0.292893 7.29289L6.65685 0.928932C7.04738 0.538408 7.68054 0.538408 8.07107 0.928932C8.46159 1.31946 8.46159 1.95262 8.07107 2.34315L2.41421 8L8.07107 13.6569C8.46159 14.0474 8.46159 14.6805 8.07107 15.0711C7.68054 15.4616 7.04738 15.4616 6.65685 15.0711L0.292893 8.70711ZM17 9H1V7H17V9Z" fill="#878787"/></svg></button>',
		responsive: [
		{
			breakpoint: 1170,
			settings: {
